...
const Favorite: React.FC<FavoriteProps> = ({ contentId }) => {
  ...
  const toggleFavorite = async () => {
    try {
      const endpoint = isFavorite ? '/favorites/video/remove' : '/favorites/video/add';
  ...
  return (
    <div onClick={toggleFavorite} className="cursor-pointer transition-colors duration-300">
      <svg
